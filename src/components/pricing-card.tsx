  ...
  return (
    ...
    >
      ...
      <div className="mt-4 flex items-end gap-2">
        <span className="text-4xl font-display uppercase tracking-wide text-sand">
          {tier.price}
        </span>
      ...
      <p className="mt-4 text-sm text-sand/70">{tier.description}</p>
      <ul className="mt-6 flex list-disc flex-col gap-2 pl-5 text-sm text-sand/70">
        {tier.features.map((feature) => (
          <li key={feature}>{feature}</li>
        ))}
      </ul>
      <div className="mt-6">
        <BookTrialButton className="w-full rounded-full bg-ember px-6 py-3 text-xs font-semibold uppercase tracking-[0.35em] text-white" />
      </div>
    </div>
  );
}
